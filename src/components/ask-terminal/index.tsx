...
export default function AskTerminal(props: AskTerminalProps) {
  ...
  return (
    ...
    >
      {loading && (
        <div className="absolute inset-0 z-20 flex flex-col gap-3 items-center justify-center text-white/70">
          <Loader />
      ...
      {status.healthy &&
        ...
        terminalState.connectionVersion === 0 && (
          <div className="absolute inset-0 z-20 flex flex-col gap-3 items-center justify-center text-white/70">
            <svg
              xmlns="http://www.w3.org/2000/svg"
              width="24"
            ...
            <div className="flex flex-col items-center gap-1 max-w-[250px] text-center">
              <span className="text-white">Welcome to the Ask Terminal!</span>
              <span className="text-sm">
                Explore the terminal by running commands. To get started, press
                the button below.
              </span>
            </div>
            <button
      ...
      {terminalState?.status === "disconnected" &&
        terminalState.connectionVersion === 1 && (
          <div className="absolute inset-0 z-20 flex flex-col gap-3 items-center justify-center text-white/70">
            <svg
              xmlns="http://www.w3.org/2000/svg"
              width="24"
            ...
            <div className="flex flex-col items-center gap-1 max-w-[250px] text-center">
              <span className="text-white">Oops! Connection lost.</span>
              <span className="text-sm">
                Don't worry, you can reconnect to the terminal supafast.
              </span>
            </div>
            <button
              onClick={terminalRef.current?.reconnect}
      ...
      <div
        className={`transition duration-500 ${status.healthy && !loading && terminalState?.status === "connected" ? "opacity-100" : "opacity-0"}`}
      >
        <XTerm onStateChange={setTerminalState} ref={terminalRef} />
      </div>
      <CommandShortcuts
        className={`transition duration-500 border-t border-white/20 ${status.healthy && !loading && terminalState?.status === "connected" ? "opacity-100" : "opacity-0"}`}
        onCommand={(command) => {
